 ...
 *
 * Two things keep a stored value honest. It is **written only for a registered
 ...
 * removed is dropped rather than navigated to. And because that check resolves
 * against the registry, what comes back is a `NavPath`: the caller gets something
 * a `Link` will accept, not a string it has to cast.
 *
 ...
 * to. Both rails are recorded the same way, which is what makes the memory right
...
export function lastLocation(
  ...
  // been removed or moved to the other rail since it was stored would otherwise
  // send someone to the shell's "not available here" panel, or to the wrong rail.
