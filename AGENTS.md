...
- **Hosted** (`OTARI_MODE=hosted`) is standalone's multi-tenant variant, not a third runtime: same database, same management routers, same sign-in, and `is_hybrid_mode` is false. Two things differ. It serves **no data plane**: `_register_data_plane_routers` (`src/gateway/api/main.py`) mounts none of the inference routers (chat/messages/responses, embeddings, images, audio, rerank, moderations, search, batches, files), and `api/routes/hosted_mode.py` answers those paths with a 404 naming `data_plane_url`, because an organization's wallet is debited by the usage a data-plane gateway reports *to* this control plane, so inference served here is unbilled (#822). And `GET /v1/bootstrap` publishes `deployment_type: "hosted"` with `HOSTED_SURFACES` rather than `STANDALONE_SURFACES`, which drops the process-global `providers` page (`provider_credentials` is keyed on instance name alone, so one row serves every tenant) and adds the organization-scoped `organization_providers` one. Hiding a page is not a guard over the table; #818 tracks that.
